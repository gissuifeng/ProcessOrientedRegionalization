...
def hierarchical_clustering(similarity_measuring: str, time_series_dict: Dict[str, List[Tuple[float, str]]],
    ...
    """
        ...
        :param neighbor_dict_raw: 原始邻居信息字典
        :param k: 聚类停止的阈值，默认为10
        :param min_regions: 最小区域数，用于强制合并小聚类，默认为5
        :return: 聚类后的时间序列字典
    """
    # 初始化簇，每个时间序列作为一个独立的簇
    ...
    def get_similarity(id1: str, id2: str) -> float:
        """
        计算两个聚类之间的相似度
        使用 lru_cache 来缓存结果，避免重复计算
        """
        cluster1, cluster2 = clusters[id1], clusters[id2]
        if len(cluster1.members) == 1 and len(cluster2.members) == 1:
            # 如果两个聚类都只包含一个序列，直接计算相似度
            return round(eval(similarity_measuring)(cluster1.data[0], cluster2.data[0]), 6)
        else:
            # 如果至少有一个聚类包含多个序列，随机选择序列计算平均相似度
            similarities = [eval(similarity_measuring)(s1, s2)
                            for s1 in random.sample(cluster1.data, min(10, len(cluster1.data)))
                            for s2 in random.sample(cluster2.data, min(10, len(cluster2.data)))]
            return sum(similarities) / len(similarities)


    # 判断是否应该强制合并两个聚类
    def should_force_merge(cluster1, cluster2):
        """
                判断是否应该强制合并两个簇。
        ...
        cluster1_neighbors = set()
        for item in cluster1.id.split('_'):
            cluster1_neighbors.update(neighbor_dict_raw[item])
        return cluster1_neighbors - set(cluster1.id.split('_')) <= set(cluster2.id.split('_'))
    ...
    similarity_heap = [(get_similarity(id1, id2), id1, id2)
                       for id1 in clusters
    ...
    while len(clusters) > k:
        ...
        for id1 in clusters:
            for id2 in neighbor_dict[id1]:
                if id2 in clusters and should_force_merge(clusters[id1], clusters[id2]):
        ...
        if not force_merge and len(clusters) < 2 * k:
            small_clusters = [id for id, cluster in clusters.items() if len(cluster.members) < min_regions]
            if small_clusters:
                id1 = small_clusters[0]
                best_neighbor = min((id2 for id2 in neighbor_dict[id1] if id2 in clusters), key=lambda id2: get_similarity(id1, id2))
                force_merge = (id1, best_neighbor, get_similarity(id1, best_neighbor))

        if force_merge:
            id1, id2, similarity = force_merge
            # reason = "全包围" if should_force_merge(clusters[id1], clusters[id2]) else f"簇大小 ({count_regions(clusters[id1])} < {min_regions})"
            # print(f"强制合并区域 {sorted(clusters[id1].id.split('_'))} 到 {sorted(clusters[id2].id.split('_'))},相似性：{similarity}（{reason}）")
        elif similarity_heap:
            similarity, id1, id2 = heapq.heappop(similarity_heap)
        else:
            break
        ...
        # print(f"合并区域 {sorted(clusters[id1].id.split('_'))} 和 {sorted(clusters[id2].id.split('_'))}, 相似度: {similarity}，当前区域数量: {len(clusters)}")
        ...
        for neighbor in new_neighbors:
            if neighbor in clusters:
                similarity = get_similarity(new_cluster.id, clusters[neighbor].id)
                heapq.heappush(similarity_heap, (similarity, min(new_cluster.id, neighbor), max(new_cluster.id, neighbor)))
...
def script_tool(sts_param0, neighborMetrix_param1):
    ...
    result = hierarchical_clustering(method[2], time_series_dict, neighbor_dict, neighbor_dict_raw, k=4, min_regions=5)# k结果中簇的数量，min_regions：每个簇中预期最少区域的数量（算法会尽量满足）
    ...
    # 结果可视化
    #visualize_clusters(time_series_dict, result.keys())
...
if __name__ == "__main__":
    ...
    sts_param0 = arcpy.GetParameterAsText(0)
    neighborMetrix_param1 = arcpy.GetParameterAsText(1)

    script_tool(sts_param0, neighborMetrix_param1)
