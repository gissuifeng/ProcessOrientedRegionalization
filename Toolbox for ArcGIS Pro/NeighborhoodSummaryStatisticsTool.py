...
def calculate_statistics(neighbor_csv_file, time_series_dict, stat_type, similarity_method):
    # 用于存储每个区域及其邻居的字典
    regions = {}
    
    # 读取 CSV 文件
    with open(neighbor_csv_file, 'r') as file:
    ...
    for region, neighbors in regions.items():
        ...
        for neighbor in neighbors:
            if region in time_series_dict and neighbor in time_series_dict:

                similarity = 0.0

                if similarity_method == "European_Distance_based_Value_Similairty":
                    similarity = compute_Edis_similarity(time_series_dict[region], time_series_dict[neighbor])

                if similarity_method == "DTW_based_Value_Similarity":
                    similarity = compute_Edis_similarity(time_series_dict[region], time_series_dict[neighbor])

                if similarity_method == "Cosine_based_Shape_Similarity":
                    similarity = calculate_shape_similarity(time_series_dict[region], time_series_dict[neighbor])

                if similarity_method == "Fourier_based_Period_Similarity":
                    similarity = compute_period_similarity(time_series_dict[region], time_series_dict[neighbor])
                
                similarities.append(similarity)
        
        if similarities:
            if stat_type == 'Mean':
                stat_value = mean(similarities)
            elif stat_type == 'Variance':
                stat_value = variance(similarities) if len(similarities) > 1 else 0
            else:
                raise ValueError("统计类型必须是 'mean' 或 'variance'")
        else:
            stat_value = None  # 如果没有有效的邻居数据
        
        result[region] = stat_value

    return result

def copy_feature_class_and_add_similarity(original_fc, new_fc, stat_dic, field_name):
    # 复制原始要素类
    ...
    with arcpy.da.UpdateCursor(new_fc, ["region_id", field_name]) as cursor:
        for row in cursor:
            ...
            if region_id in stat_dic:
                row[1] = stat_dic[region_id]
...
def script_tool(input_areal_featurelayer, spatial_time_series, spatial_neighbor_metrix, similarity_method, statistical_method, output_featureclass):
    """Script code goes below"""

    time_series_dic = {}
    time_series_dic = create_lengthwise_time_series_list(spatial_time_series)

    # arcpy.AddMessage(time_series_dic)

    result_dic = {}
    result_dic = calculate_statistics(spatial_neighbor_metrix, time_series_dic, statistical_method, similarity_method)

    arcpy.AddMessage(result_dic)

    copy_feature_class_and_add_similarity(input_areal_featurelayer, output_featureclass, result_dic, statistical_method)
...
if __name__ == "__main__":
    ...
    spatial_time_series = arcpy.GetParameterAsText(1)
    spatial_neighbor_metrix = arcpy.GetParameterAsText(2)
    similarity_method = arcpy.GetParameterAsText(3)
    statistical_method = arcpy.GetParameterAsText(4)
    output_featureclass = arcpy.GetParameterAsText(5)
    

    script_tool(input_areal_featurelayer, spatial_time_series, spatial_neighbor_metrix, similarity_method, statistical_method, output_featureclass)
